resource "azurerm_resource_group_template_deployment" "SQLMI_Secondary" {
...

{
	...
	"resources": [{
		...
		{
			"condition": "[equals(parameters('environment'),'R')]",
...
resource "null_resource" "secondary_aad_group_member" {
  count = local.is_secondary ? 1 : 0

  provisioner "local-exec" {
    command = "az login --identity --output none; az ad group member add --group 6820692a-6eda-40be-8c27-a4007d926a97 --member-id ${jsondecode(azurerm_resource_group_template_deployment.SQLMI_Secondary.0.output_content).principal_id.value}"
  }
  depends_on = [azurerm_resource_group_template_deployment.SQLMI_Secondary]
}

resource "time_sleep" "secondary_wait_30_seconds" {
  count           = local.is_secondary ? 1 : 0
  create_duration = "30s"
  depends_on      = [null_resource.secondary_aad_group_member]
}

resource "null_resource" "secondary_aad_admin" {
  count = local.is_secondary ? 1 : 0
  ...
  depends_on = [time_sleep.secondary_wait_30_seconds]
}