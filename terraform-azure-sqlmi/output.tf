...
output "standalone_instance_name" {
  value = try(
    jsondecode(azurerm_resource_group_template_deployment.SQLMI_Standalone.0.output_content).name.value, 
    null
  )
}
...
output "standalone_instance_host" {
   value = try(
    jsondecode(azurerm_resource_group_template_deployment.SQLMI_Standalone.0.output_content).host.value, 
    null
  )
}
...
output "standalone_instance_principal_id" {
   value = try(
    jsondecode(azurerm_resource_group_template_deployment.SQLMI_Standalone.0.output_content).principal_id.value, 
    null
  )
}
