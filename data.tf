...
data "azurerm_shared_image" "windowssharedimage" {
...
data "azurerm_key_vault" "keyvault_windows" {
  provider            = azurerm.GO01
...
data "azurerm_key_vault_secret" "keyvault_secret" {
  provider     = azurerm.GO01
  name         = "adminPassword"
  key_vault_id = data.azurerm_key_vault.keyvault_windows.id
}
...
data "azurerm_key_vault_secret" "domain_secret" {
  provider     = azurerm.GO01
